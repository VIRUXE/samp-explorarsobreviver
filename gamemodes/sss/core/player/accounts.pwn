...
#define FIELD_PLAYER_HARDWAREID		"hardwareid"	// 19
...
hook OnGameModeInit()
{
	...
	//stmt_AccountGetAliasData	= db_prepare(gAccounts, "SELECT "FIELD_PLAYER_IPV4", "FIELD_PLAYER_PASS", "FIELD_PLAYER_HARDWAREID" FROM "ACCOUNTS_TABLE_PLAYER" WHERE "FIELD_PLAYER_NAME"=? AND "FIELD_PLAYER_ACTIVE" COLLATE NOCASE");
}

hook OnPlayerConnect(playerid)
{
	dbg("global", LOG_CORE, "[OnPlayerConnect] in /gamemodes/sss/core/player/accounts.pwn");
...
/*	if(CallLocalFunction("OnPlayerLoadAccount", "d", playerid))
	...
	if(!exists)
	{
		log(DISCORD_CHANNEL_EVENTS, "[ACCOUNT] `%p` does not exist.", playerid);
		return 0;
	}
	...
	log(DISCORD_CHANNEL_EVENTS, "[ACCOUNT] `%p` exists, prompting login. (Alive: `%s`, Last login: `%T`)", playerid, alive ? ("Yes") : ("No"), lastlog);
...
CreateAccount(playerid, password[])
{
	...
	log(DISCORD_CHANNEL_EVENTS, "[ACCOUNT] `%p` has registered.", playerid);
	...
	stmt_bind_value(stmt_AccountCreate, 5, DB::TYPE_STRING,		hwid); 

	if(!stmt_execute(stmt_AccountCreate))
	{
		err("[CreateAccount] executing statement `stmt_AccountCreate`.");
		KickPlayer(playerid, "An error occurred while executing statement 'stmt_AccountCreate'. Please contact an admin on Discord (chat.viruxe.party).");
		return 0;
	}
...
DisplayRegisterPrompt(playerid)
{
	...
	log(DISCORD_CHANNEL_ADMINEVENTS, "[ACCOUNT] `%p` is registering.", playerid);

	inline Response(pid, dialogid, response, listitem, string:inputtext[])
	{
		#pragma unused pid, dialogid, listitem

		if(response) // Entered password
		{
		...
		{
			log(DISCORD_CHANNEL_ADMINEVENTS, "[ACCOUNTS] `%p` left the server without registering.", playerid);
			KickPlayer(playerid, "Not wanting to register", true);
		}
		...
		if(response)
		{
			if(strlen(inputtext) < 4)
			{
				...
				if(acc_LoginAttempts[playerid] < 5)
				{
					...
					log(DISCORD_CHANNEL_GLOBAL, "[PART] `%p` left the server without logging in.", playerid), Kick(playerid);
...
{
	new 
		spawntime = GetPlayerCreationTimestamp(playerid),
		daysalive = (gettime() > spawntime) ? 0 : (gettime() - spawntime) / 86400,
		alvl;
	...
	alvl = GetPlayerAdminLevel(playerid);
	if(alvl)
	{
		new
		...
		ChatMsg(playerid, BLUE, " >  Admin Level: %d (%s)", alvl, GetAdminRankName(alvl));
	...
	if(daysalive)
	{
		SetPlayerScore(playerid, daysalive);
		ChatMsg(playerid, BLUE, " >  You have been alive for %d day(s).", daysalive);
	}
	...
	log(DISCORD_CHANNEL_GLOBAL, "[ACCOUNT] `%p` has logged in. (Alive: %s)", playerid, IsPlayerAlive(playerid) ? ("Yes") : ("No"));
	//DiscordMessage(DISCORD_CHANNEL_GLOBAL, "%s (%d) `%p` has logged in.%s", alvl > 0 ? (sprintf("@here %s ", GetAdminRankName(alvl))) : (""), playerid, playerid, daysalive ? sprintf(" (Days alive: %d)", daysalive) : (""));
	
	if(alvl)
		DiscordMessage(DISCORD_CHANNEL_ADMIN, "(%d) `%p` has logged in.", playerid, playerid);
}
...
Logout(playerid, docombatlogcheck = 1)
{
	...
	{
		log(DISCORD_CHANNEL_EVENTS, "[ACCOUNT] `%p` was not logged in.", playerid);
		return 0;
	}
	...
	log(DISCORD_CHANNEL_EVENTS, "[ACCOUNT] `%p` logged out at `%.0f, %.0f, %.0f` (Logged In: `%s`, Alive: `%s`, Knocked Out: `%s`)",
		playerid, x, y, z, acc_LoggedIn[playerid] ? ("Yes") : ("No"), IsPlayerAlive(playerid) ? ("Yes") : ("No"), IsPlayerKnockedOut(playerid) ? ("Yes") : ("No"));

	if(IsPlayerOnAdminDuty(playerid))
	{
		dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[LOGOUT_] ERROR: Player on admin duty, aborting save.");
		return 0;
	}
	...
	if(docombatlogcheck)
	{
		if(gServerMaxUptime - gServerUptime > 30)
		{
			...
			if(IsPlayerCombatLogging(playerid, lastattacker, lastweapon))
			{
				...
				log(DISCORD_CHANNEL_ADMINEVENTS, "[ACCOUNT] '%p' combat logged!", playerid);
	...
	{
		dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[ACCOUNT] Player is holding a box.");
		if(!IsContainerEmpty(GetItemExtraData(itemid)))
		{
			dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[ACCOUNT] Player is holding an unempty box, dropping in world.");
	...
	{
		dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[ACCOUNT] Player is holding a bag.");
		if(!IsContainerEmpty(GetItemArrayDataAtCell(itemid, 1)))
		{
			if(IsValidItem(GetPlayerBagItem(playerid)))
			{
				dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[ACCOUNT] Player is holding an unempty bag and is wearing one, dropping in world.");
				CreateItemInWorld(itemid, x + floatsin(-r, degrees), y + floatcos(-r, degrees), z - FLOOR_OFFSET);
				itemid = INVALID_ITEM_ID;
				itemtype = INVALID_ITEM_TYPE;
			}
			...
			{
				dbg("gamemodes/sss/core/player/accounts.pwn", 1, "[ACCOUNT] Player is holding an unempty bag but is not wearing one, calling GivePlayerBag.");
...
stock GetAccountData(name[], pass[], &ipv4, &alive, &regdate, &lastlog, &spawntime, &totalspawns, &warnings, hardwareid[], &active)
{
	stmt_bind_value(stmt_AccountLoad, 0, DB::TYPE_STRING, name, MAX_PLAYER_NAME);
	...
	stmt_bind_result_field(stmt_AccountLoad, FIELD_ID_PLAYER_WARNINGS, DB::TYPE_INTEGER, warnings);
	stmt_bind_result_field(stmt_AccountLoad, FIELD_ID_PLAYER_HARDWAREID, DB::TYPE_STRING, hardwareid, 32);

...
{
	dbg("global", LOG_CORE, "[OnPlayerConnect] in /gamemodes/sss/core/weapon/damage-core.pwn");
...
stock PlayerInflictWound(playerid, targetid, E_WND_TYPE:type, Float:bleedrate, Float:knockmult, calibre, bodypart, source[])
{
	...
	{
		log(DISCORD_CHANNEL_DEV, "[WARNING] Knocked out player `%p` tried to wound player `%p` at `%f, %f, %f`", playerid, targetid, x,y,z);
		return 0;
	}
	...
	if(IsPlayerConnected(playerid))
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[DAMAGE] `%p` wounded `%p` (Bleedrate %f, Knockmult %f, Bodypart %d, Source `%s`) at `%.0f, %.0f, %.0f`", playerid, targetid, bleedrate, knockmult, bodypart, source, x,y,z);
	}
	else
		log(DISCORD_CHANNEL_EVENTS, "[DAMAGE] `%p` is wounded (Bleedrate %f, Knockmult %f, Bodypart %d, Source `%s`) at `%.0f, %.0f, %.0f`", targetid, bleedrate, knockmult, bodypart, source, x,y,z);
...
{
	dbg("global", LOG_CORE, "[OnDeath] in /gamemodes/sss/core/weapon/damage-core.pwn");
...
{
	dbg("global", LOG_CORE, "[OnPlayerSave] in /gamemodes/sss/core/weapon/damage-core.pwn");
...
{
	dbg("global", LOG_CORE, "[OnPlayerLoad] in /gamemodes/sss/core/weapon/damage-core.pwn");

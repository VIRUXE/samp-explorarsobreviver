...
hook OnHoldActionFinish(playerid)
{
	if(def_CurrentDefenceItem[playerid] != INVALID_ITEM_ID)
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[CONSTRUCT] `%p` Built defence %d (%s) %d at `, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f`",
	...
	if(def_CurrentDefenceEdit[playerid] != -1)
	{
		...
		if(itemtype == item_Crowbar)
		{
			...
			log(DISCORD_CHANNEL_EVENTS, "[CROWBAR] `%p` broke defence %d (%s) `%d, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f`",
	...
	if(keypadid == 100)
	{
		...
		if(def_CurrentDefenceOpen[playerid] != -1)
		{
			...
			else
			{
				...
				log(DISCORD_CHANNEL_ADMINEVENTS, "[DEFENCE] `%p` entered the wrong code (`%d`) on defence %d (`%.0f, %.0f, %.0f`)", playerid, code, def_CurrentDefenceOpen[playerid], x,y,z);
	...
	inline Response(pid, dialogid, response, listitem, string:inputtext[])
	{
		...
		if(response)
		{
			...
			else
			{
				...
				log(DISCORD_CHANNEL_ADMINEVENTS, "[DEFENCE] `%p` entered the wrong pass (`%d`) on defence %d (%s)", playerid, pass, def_CurrentDefenceOpen[playerid], geid);
...
timer MoveDefence[1500](itemid, playerid)
{
	...
	if(GetItemArrayDataAtCell(itemid, def_pose) == DEFENCE_POSE_HORIZONTAL)
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[DEFENCE] `%p` moved defence %d (%s) into CLOSED position at `%.0f, %.0f, %.0f`", playerid, itemid, geid, ix, iy, iz);
	...
	else
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[DEFENCE] `%p` moved defence %d (%s) into OPEN position at `%.0f, %.0f, %.0f`", playerid, itemid, geid, ix, iy, iz);
		CallLocalFunction("OnDefenceMove", "d", itemid);
	}
...
hook OnItemDestroy(itemid)
{
	...
	if(def_ItemTypeDefenceType[itemtype] != -1)
	{
		if(GetItemHitPoints(itemid) <= 0)
		{
			...
			log(DISCORD_CHANNEL_EVENTS, "[DESTRUCTION] Defence %d (%d) Object: (%d, %f, %f, %f, %f, %f, %f)", itemid, _:itemtype, GetItemTypeModel(itemtype), x, y, z, rx, ry, rz);
		}
	}
}

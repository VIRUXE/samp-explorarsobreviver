...
{
	dbg("global", LOG_CORE, "[OnPlayerConnect] in /gamemodes/sss/core/admin/core.pwn");
...
{
	dbg("global", LOG_CORE, "[OnPlayerDisconnected] in /gamemodes/sss/core/admin/core.pwn");
...
TimeoutPlayer(playerid, reason[])
{
	...
	log(DISCORD_CHANNEL_EVENTS, "[PART] `%p` (timeout: `%s`)", playerid, reason);
...
KickPlayer(playerid, reason[], bool:tellplayer = true)
{
	...
	log(DISCORD_CHANNEL_GLOBAL, "[PART] `%p` was kicked. Reason: `%s`", playerid, reason);
...
TogglePlayerAdminDuty(playerid, bool:exitAtSameLocation = false)
{
	...
	if(!IsPlayerOnAdminDuty(playerid))
	{
		...
		Logout(playerid, .docombatlogcheck = 0);
		...
		log(DISCORD_CHANNEL_ADMINEVENTS, "[DUTY] `%p` entered admin duty at `%.0f, %.0f, %.0f` (%s).", playerid, x, y, z, zone);
	}
	else
	{
		...
		log(DISCORD_CHANNEL_ADMINEVENTS, "[DUTY] `%p` exited admin duty at `%.0f, %.0f, %.0f` (%s).", playerid, x, y, z, zone);
	}
}

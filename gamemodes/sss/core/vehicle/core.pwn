...
{
	dbg("global", LOG_CORE, "[OnPlayerConnect] in /gamemodes/sss/core/vehicle/core.pwn");
	...
	veh_DamageUI[playerid]			=CreatePlayerTextDraw(playerid, 620.000000, 371.000000, "DAMAGE");
	...
	veh_EngineUI[playerid]			=CreatePlayerTextDraw(playerid, 620.000000, 356.000000, "ENGINE");
	...
	veh_DoorsUI[playerid]			=CreatePlayerTextDraw(playerid, 620.000000, 341.000000, "LOCKED");
...
IsVehicleEngineAtTheBack(vehicleid)
{
	new vehicles[] = 
	{
		568,571,539,530,424,
		493,484,473,472,452,453,454,446,430
	};
...
IsVehicleColourInvalid(colour)
{
	new invalidColours[] = // Mostly pink colours, ordered per row from https://wiki.sa-mp.com/wiki/Color_ID
	{
		134,136,139,143,
		144,146,147,
		167,169,170,
	...
	for(new i; i < sizeof(invalidColours); i++)
	{
		if(colour == invalidColours[i])
			return 1;
	}
...
GetRandomVehicleColour()
{
	new colour = 128 + random(128);

	while(IsVehicleColourInvalid(colour))
		colour = 128 + random(128);
...
stock CreateWorldVehicle(type, Float:x, Float:y, Float:z, Float:r, colour1, colour2, world = 0, geid[GEID_LEN] = "")
{
	...
	//log(DISCORD_CHANNEL_DEV, "[CreateWorldVehicle] Creating vehicle of type %d (%s %d) at %.0f, %.0f, %.0f with colours %d %d", type, GetVehicleName(veh_Data[type][veh_modelId]), veh_Data[type][veh_modelId], x, y, z, colour1, colour2);
...
stock DestroyWorldVehicle(vehicleid, bool:perma = false)
{
	...
	if(perma)
	{
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] Permanently destroying `%s (%d)` at `%.0f, %.0f, %.0f`", vehiclename, vehicleid, x,y,z);
		DestroyVehicle(vehicleid);
	}
	else
	{
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] Destroying `%s (%d)` at `%.0f, %.0f, %.0f`", vehiclename, vehicleid, x,y,z);
...
stock ResetVehicle(vehicleid)
{
	...
	log(DISCORD_CHANNEL_EVENTS, "[ResetVehicle] Resetting vehicle %d, new ID: %d", vehicleid, newid);
...
_veh_create(type, Float:x, Float:y, Float:z, Float:r, colour1, colour2, world = 0, geid[GEID_LEN] = "")
{
	...
	if(IsVehicleColourInvalid(vc1))
		vc1 = GetRandomVehicleColour();

	if(IsVehicleColourInvalid(vc2))
		vc2 = GetRandomVehicleColour();

	vehicleid = CreateVehicle(GetVehicleTypeModel(type), x, y, z, r, vc1, vc2, -1);
...
{
	dbg("global", LOG_CORE, "[OnPlayerKeyStateChange] in /gamemodes/sss/core/vehicle/core.pwn");
...
PlayerVehicleUpdate(playerid)
{
	...
	if(maxfuel > 0.0) // If the vehicle is a fuel powered vehicle
	{
		...
		format(str, sizeof(str), "%.2f/%.2fL FUEL", GetVehicleFuel(vehicleid), maxfuel);
...
{
	dbg("global", LOG_CORE, "[OnPlayerStateChange] in /gamemodes/sss/core/vehicle/core.pwn");
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] `%p` entered `%s (%d)` as driver at `%.0f, %.0f, %.0f`", playerid, GetVehicleGEID(veh_Current[playerid]), veh_Current[playerid], x, y, z);
	}
	...
	if(oldstate == PLAYER_STATE_DRIVER)
	{
		...
		{
			// Happens when you delete a vehicle whilst being the driver
		...
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] `%p` exited `%s (%d)` as a Driver at `%.0f, %.0f, %.0f`", playerid, GetVehicleGEID(veh_Current[playerid]), veh_Current[playerid], veh_Data[veh_Current[playerid]][veh_spawnX], veh_Data[veh_Current[playerid]][veh_spawnY], veh_Data[veh_Current[playerid]][veh_spawnZ]);
	}
	...
	if(newstate == PLAYER_STATE_PASSENGER)
	{
		...
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] `%p` entered `%s (%d)` as a Passenger at `%.0f, %.0f, %.0f`", playerid, vehiclename, veh_Current[playerid], x, y, z);
	}
	...
	if(oldstate == PLAYER_STATE_PASSENGER)
	{
		...
		HideVehicleUI(playerid);
		log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] `%p` exited `%s (%d)` as a Passenger at `%.0f, %.0f, %.0f`", playerid, vehiclename, veh_Current[playerid], x, y, z);
	}
...
{
	dbg("global", LOG_CORE, "[OnUnoccupiedVehicleUpd] in /gamemodes/sss/core/vehicle/core.pwn");
...
public OnVehicleDeath(vehicleid, killerid)
{
	...
	log(DISCORD_CHANNEL_EVENTS, "[VEHICLE] Vehicle `%s (%d)` killed by `%p` at `%.0f, %.0f, %.0f`", GetVehicleName(vehicleid), vehicleid, killerid, veh_Data[vehicleid][veh_spawnX], veh_Data[vehicleid][veh_spawnY], veh_Data[vehicleid][veh_spawnZ]);
}
...
public OnVehicleSpawn(vehicleid)
{
	if(veh_Data[vehicleid][veh_state] == VEHICLE_STATE_DYING)
	{
		...
		{
			log(DISCORD_CHANNEL_ADMINEVENTS, "[VEHICLE] Dead Vehicle (`%s %d`) spawned out of bounds (`%.0f, %.0f, %.0f`). Probably glitched vehicle death, respawning.", GetVehicleName(vehicleid), vehicleid, x,y,z);
		...
		{
			log(DISCORD_CHANNEL_ADMINEVENTS, "[VEHICLE] Dead Vehicle (`%s %d`) spawned at `%.0f, %.0f, %.0f`. Setting as inactive.", GetVehicleName(vehicleid), vehicleid, x,y,z);
...
stock vti_CreateVehicle(vehicletype, Float:x, Float:y, Float:z, Float:rotation, color1, color2, respawn_delay)
{
	...
	err(DISCORD_CHANNEL_DEV, "Cannot create vehicle by model ID.");
